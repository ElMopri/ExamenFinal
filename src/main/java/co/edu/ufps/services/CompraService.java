...
import co.edu.ufps.dto.DetallesCompraResponseDTO;
...
public class CompraService {
	...
	public CompraDTO crear(String uuid, CompraRequestDTO compraRequestDTO) {
		...
		if (tiendaOpt.isPresent()) {
			...
			Cliente cliente = clienteRepository.findByDocumentoAndTipoDocumento_Nombre(
					compraRequestDTO.getCliente().getDocumento(), compraRequestDTO.getCliente().getTipo_documento());
			if (cliente != null) { // el cliente ya existe
				...
				TipoDocumento tipoDocumento = tipoDocumentoRepository
						.findByNombre(compraRequestDTO.getCliente().getTipo_documento());
			...
			for (DetallesCompraDTO detallesCompraDTO : compraRequestDTO.getProductos()) {
				...
				if (producto != null) {
					if (detallesCompraDTO.getCantidad() <= producto.getCantidad()) {
						...
						BigDecimal descuentoProducto = producto.getPrecio()
								.multiply(detallesCompraDTO.getDescuento().divide(cien));
						BigDecimal productoPrecio = producto.getPrecio().subtract(descuentoProducto);

						BigDecimal productoFinal = productoPrecio
								.multiply(new BigDecimal(detallesCompraDTO.getCantidad()));

						BigDecimal impuestoProductoFinal = productoFinal.multiply(compraRequestDTO.getImpuesto().divide(cien));

						compra.setTotal(compra.getTotal().add(productoFinal.add(impuestoProductoFinal)));
						
						compra.setImpuestos(compra.getImpuestos().add((impuestoProductoFinal)));

					} else {
						compraDTO.setStatus("403");
						compraDTO.setMessage("La cantidad a comprar de " + producto.getNombre()
								+ " supera el máximo del producto en tienda");
						return compraDTO; // Status 403
					}
				} else {
					compraDTO.setStatus("404");
					compraDTO.setMessage("La referencia del producto " + detallesCompraDTO.getReferencia()
	...
	public FacturaResponseDTO consultar(String uuid, FacturaRequestDTO facturaRequestDTO) {
		Optional<Tienda> tiendaOpt = tiendaRepository.findFirstByUuid(uuid);
		if (!tiendaOpt.isPresent()) {
			return null;
		}
		...
		Optional<Compra> compraOpt = compraRepository.findById(facturaRequestDTO.getFactura());
		...
		facturaResponseDTO.setTotal(compra.getTotal());
		
		facturaResponseDTO.setImpuestos(compra.getImpuestos());
		
		if (facturaRequestDTO.getCliente() == null || facturaRequestDTO.getTipo_documento() == null) {
			return null; // Sin informacion suficiente del cliente
		}
		Cliente cliente = clienteRepository.findByDocumentoAndTipoDocumento_Nombre(facturaRequestDTO.getCliente(), facturaRequestDTO.getTipo_documento());
		if (cliente == null) {
			return null; // El cliente no existe
		}
		
		facturaResponseDTO.getCliente().setDocumento(cliente.getDocumento());
		facturaResponseDTO.getCliente().setNombre(cliente.getNombre());
		facturaResponseDTO.getCliente().setTipo_documento(cliente.getTipoDocumento().getNombre());
		
		List<DetallesCompraResponseDTO> detallesCompraResponseDTO = new ArrayList<DetallesCompraResponseDTO>();
		
		for(DetallesCompra producto : compra.getDetallesCompras()) {
			DetallesCompraResponseDTO productoDTO = new DetallesCompraResponseDTO();
			productoDTO.setReferencia(producto.getProducto().getReferencia());
			productoDTO.setNombre(producto.getProducto().getNombre());
			productoDTO.setCantidad(producto.getCantidad());
			productoDTO.setPrecio(producto.getPrecio());
			
			BigDecimal descuento = ((producto.getPrecio().multiply(producto.getDescuento())).divide(new BigDecimal(100)));
			
			descuento = descuento.multiply(new BigDecimal(producto.getCantidad()));
			
			productoDTO.setDescuento(descuento);
			
			BigDecimal subtotal = (producto.getPrecio().multiply(new BigDecimal(producto.getCantidad())));
			
			subtotal = subtotal.subtract(descuento);
			
			productoDTO.setSubtotal(subtotal);
			
			detallesCompraResponseDTO.add(productoDTO);
		}
		
		facturaResponseDTO.setProductos(detallesCompraResponseDTO);
		
		if (facturaRequestDTO.getToken() == null) {
			return null; // Sin informacion suficiente del cajero
		}
		Cajero cajero = cajeroRepository.findByToken(facturaRequestDTO.getToken());
		if (cajero == null) {
			return null; // El cliente no existe
		}
		
		facturaResponseDTO.getCajero().setDocumento(cajero.getDocumento());
		facturaResponseDTO.getCajero().setNombre(cajero.getNombre());
